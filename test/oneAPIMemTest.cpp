...
process(queue& q, uint64_t globalSize, uint64_t allocSize, bool random, uint32_t duration, uint32_t id=0, bool verbose = true) {
    ...
    while( duration==0 || (timeElapsed(startTime)/1000 < duration ) ) {
		...
		if (!res) {
			std::cout << BLUE << "\t\tMemory Read failed" << RESET << std::endl;
	     		//return false;
		}
...
int main(int argc, char* argv[]) {
    ...
    std::cout <<"\t"<<"\tRequired mem size   :\t"<< std::fixed<<std::setprecision(2) << color(id) << (float)memRequiredSize/GB <<" Gb" << RESET << std::endl;
